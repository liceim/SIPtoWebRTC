...
func (element *ConfigST) coDe(suuid string) {
	element.mutex.Lock()
	defer element.mutex.Unlock()
	t := element.Streams[suuid]
	t.Codecs = nil
...
func (element *ConfigST) sstAd(suuid string) {
	ch := make(chan string)
	tmp, ok := element.Streams[suuid]
	if !ok {
		return
	}
	tmp.SSt = sipstate{c: ch}
	element.Streams[suuid] = tmp
}

func (element *ConfigST) sstGe(suuid string) chan string {
	tmp, ok := element.Streams[suuid]
	if !ok {
		return nil
	}
